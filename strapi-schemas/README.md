...
**Бакет должен быть приватным** (решение пользователя, 2026-08-09):
публичное чтение из бакета нужно запретить, иначе прокси сайта обходится
прямой ссылкой. Сайт отдаёт файлы только через `/api/media/file/[id]` и
только по сессии; блоки хранят `id`, чтобы по нему перевыпускать
presigned-ссылку (`docs/OPEN_QUESTIONS.md` №112).
